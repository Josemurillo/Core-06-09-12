...
public:
    ...
    struct instance_halls_of_reflection_InstanceMapScript : public InstanceScript
    {
            ...
            switch (type)
            {
                ...
                case DATA_LICHKING_EVENT:
                    ...
                    if(data == DONE)
                    {
                        ...
                        //DoCompleteAchievement(ACHIEV_HALLS_OF_REFLECTION_N); falta reparar
                        if(instance->IsHeroic())
                        {
                            //DoCompleteAchievement(ACHIEV_HALLS_OF_REFLECTION_H); falta reparar
        ...
        void SpawnWave(Creature* summoner)
        {
            ...
            summoner->SummonCreature(NPC_WAVE_MERCENARY, MercenarySpawnPos[index], TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 60000);

            index = urand(0, ENCOUNTER_WAVE_FOOTMAN-1);
            summoner->SummonCreature(NPC_WAVE_FOOTMAN, FootmenSpawnPos[index], TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 60000);

            index = urand(0, ENCOUNTER_WAVE_RIFLEMAN-1);
            summoner->SummonCreature(NPC_WAVE_RIFLEMAN, RiflemanSpawnPos[index], TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 60000);

            index = urand(0, ENCOUNTER_WAVE_PRIEST-1);
            summoner->SummonCreature(NPC_WAVE_PRIEST, PriestSpawnPos[index], TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 60000);

            index = urand(0, ENCOUNTER_WAVE_MAGE-1);
            summoner->SummonCreature(NPC_WAVE_MAGE, MageSpawnPos[index], TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 60000);
        }
